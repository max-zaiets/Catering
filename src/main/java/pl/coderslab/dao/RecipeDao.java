...
public class RecipeDao {
    ...
    private static final String CREATE_RECIPE_QUERY = "INSERT INTO recipe(name,ingredients,description,preparation_time,preparation,admin_id,created) VALUES (?,?,?,?,?,?,current_time);";
    private static final String DELETE_RECIPE_QUERY = "DELETE FROM recipe where id = ?;";
    private static final String FIND_ALL_RECIPE_QUERY = "SELECT * FROM recipe;";
    private static final String READ_RECIPE_QUERY = "SELECT * from recipe where id = ?;";
    private static final String UPDATE_RECIPE_QUERY = "UPDATE	recipe SET name = ? , ingredients = ?, description = ?, preparation_time = ?,preparation = ?, updated = current_time WHERE id = ?;";
    private static final String FIND_RECIPE_BY_USER = "select * from recipe join admins a on recipe.admin_id = a.id where admin_id=?";
    ...
    public static int count(Integer adminId) {
        Recipe recipe = new Recipe();
        int count = 0;
        try (Connection connection = DbUtil.getConnection();
             PreparedStatement statement = connection.prepareStatement(FIND_RECIPE_BY_USER)
        ) {
